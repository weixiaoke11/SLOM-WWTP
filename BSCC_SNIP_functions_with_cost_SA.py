...
import operator
import numpy as np
from BSCC_SNIP_astar_with_costs_SA import *
from BSCC_SNIP_costs_with_cost_SA import *
...
def fastCopyNodes(nodes):
    ...
    for index in nodes:
        ...
        copyList[index] = [i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8], l, i[10]]
    return copyList
...
def assignHighAggregatedNodes(aggregatetPoints, rasterPoints, rasterSize, minTD):
    ...
    for i in aggregatetPoints:
        ...
        for f in rasterPoints:  # Assign height
            if (f[1] <= X + rasterSize and f[1] >= X - rasterSize) and (
                ...
                if (f[1] == X) and (f[2] == Y):
                    ...
                    f_list.append(f)
                    # 加入距离权重
                    weight_list.append(((X-f[1])**2 + (Y-f[2])**2) ** -0.5)
        ...
        if len(f_list) != len(weight_list):
            raise ValueError("len(f_list) != len(weight_list)")

        # 反距离插值法计算高程
        weight_sum = sum(weight_list)
        for f_num in range(len(f_list)):
            heightSTART += f_list[f_num][3] * weight_list[f_num]/weight_sum
        # print(len(f_list), end=", ")
...
def assignHighAggregatedNodes_Orig(aggregatetPoints, rasterPoints, rasterSize, minTD):
    """
    This function assigns the correct height to aggregate nodes by searching the height of a DEM.
...
def addBuildingsFarFromRoadTo(aggregatetPoints, forSNIP):
    ...
    for i in aggregatetPoints:
        alreadyInList = 0
        for e in forSNIP:
            if forSNIP[e][1] == i[1] and forSNIP[e][2] == i[2]:
                alreadyInList = 1
                break
        if alreadyInList == 0:
...
def costToWTP(pathToWTP, edgesID, nodes, pumps, minTD, toNode, stricklerC, f_SewerCost, coefficients):
    """
    ...
    for i in pathToWTP:
        ...
        if cnt > 0:
            ...
            pipeDiameter = getPipeDiameter_Orig(totalFlow, slope, stricklerC)  # get pipe diameter
            segmentCosts, _ = calculatePipeCosts(
                distance=distanz,
                Q=totalFlow,
                pipeDiameter=pipeDiameter,
                slope=slope,
                fc_SewerCost=f_SewerCost,
                coefficients=coefficients,
            )  # segment pipe costs #could be added totalFlow, slope,
...
def costsBetweenWWTPs(pathToWTP, edgesID, nodes, inverse, pumps, minTD, stricklerC, f_SewerCost, coefficients):
    """
    ...
    for i in pathToWTP:
        ...
        if count > 0:
            ...
            pipeDiameter = getPipeDiameter_Orig(totalFlow, slope, stricklerC)
...
def appendToNetwork(sewers, arcPath):
    """
    ...
    for populatedNode in allPopNodesOntheWay[1:]:
        ...
        for entry in pathNearWTP:
            if entry[0] == populatedNode or noramlweiter == True:  # Go to populated node in path to closest WWTP
                ...
                if count > 0:
                    nodes[entry[0]][4] = FlowNewConnectedPoint + nodes[entry[0]][4]  # add flow
                if count == 0:
                    FlowNewConnectedPoint = nodes[entry[0]][8]  # Flow
                    count += 1
...
def getInflowingNodes(nodes, partOfNetwork, initialFlowWWTP, notInaNetwork):
    ...
    if len(partOfNetwork) > 1:  # only one edge
        if len(partOfNetwork) == 2:
            ...
            #         i[4] = i[4] - flowUpSTrem
            #         break
            i = nodes[partOfNetwork[0]]
            nodes[partOfNetwork[0]][4] = i[4] - flowUpSTrem
            ...
            for i in partOfNetwork:
                if counter == 2:
                    ...
                    #         if f[4] != 0:
                    #             # If current flow is zero, don't change. this means that the connection between two wwtps were unpouplated nodes
                    #             f[4] = currFlow - thirdNodeFlow
                    #             # If current flow is zero, don't change. this means that the connection between two wwtps were unpouplated nodes
                    #         break
                    f = nodes[secondID]
                    currFlow = round(f[4], 7)  # Round to 7 digits
                    if f[4] != 0:
                        nodes[secondID][4] = currFlow - thirdNodeFlow
            ...
            if i[4] != 0:
                if partOfNetwork[1] in notInaNetwork:  # Check if against flow in path
                    nodes[partOfNetwork[0]][4] = initialFlowWWTP
                else:
                    nodes[partOfNetwork[0]][4] = i[4] - flowForFirstElement
...
def changeFlowAlongPath(nodes_BI, pathBetweenWWTPs):
    ...
    for i in pathBetweenWWTPs:
        if cnt == 1:
            eintrag = nodes_BI[i]
            nodes_BI[i][4] = flowAbove + eintrag[4]  # add flowFromAbove plus inletFlow
...
def addDEMPntstoNodes(nodes, archPathListMST, boundingCandidates, fromNode, toNode, minTD):
    ...
    if len(archPathListMST) > 1:
        for edge in archPathListMST:
            ...
            #         append = 0
            #         break
            if newPoint1 in nodes:  # Check wheter already in nodes
                append = 0
            ...
            if append == 1:
                ...
                for cell in boundingCandidates:
                    if newPoint1 != fromNode and newPoint1 != toNode and cell[0] == newPoint1:
                        nodes[newPoint1] = [newPoint1, cell[1], cell[2], heightP1, 0, 0, 0, 0, 0, [], heightP1 - minTD]
            ...
            if append == 1:
                ...
                for cell in boundingCandidates:
                    if newPoint2 != fromNode and newPoint1 != toNode and cell[0] == newPoint2:
                        nodes[newPoint2] = [newPoint2, cell[1], cell[2], heightP2, 0, 0, 0, 0, 0, [], heightP1 - minTD]
    return nodes
...
def flowIfSameCheck(WWTPs, aggregatetPoints):
    ...
    sumFlowWWTP = math.ceil(sumFlowWWTP*1000)/1000
    sumFlowAggregated = math.ceil(sumFlowAggregated*1000)/1000
...
def turnNodesIntoWWTP(WWTPs, nodes, aggregatetPoints, sewers):
    ...
    for i in aggregatetPoints:
        if i[0] not in final_wwtps:
            ...
            z = nodes[i[0]]
            if z[8] > 0 and z[4] == 0 and isInSewer == 0:
                final_wwtps.append([i[0], z[8], z[1], z[2]])  # ID, flow, x, y

    return final_wwtps


def connectivityPotential(nodes, WWTPs, idWWTP, pZero, f_merge, sa_t, force_find=False):
    """
    ...
    """
    potIndex, closestDistance, potentialNode, closestWWTP = math.inf, math.inf, None, None
    selected_Index, selected_Distance, selected_Node, selected_WWTP = math.inf, math.inf, None, None
    # 是否死循环
    infinite_loop = True

    # find the closest distance and highest potIndex
    for f in WWTPs:
        if f[0] != idWWTP:
            idWWTPONE, pOne, _, _, forceConnection, _ = getPns(f[0], nodes)
            if forceConnection == 0 or force_find:
                infinite_loop = False
    ...
    # use Simulated Annealing to find the selected option
    for sa_f in WWTPs:
        if sa_f[0] != idWWTP:
            idWWTPONE, pOne, _, _, forceConnection, _ = getPns(sa_f[0], nodes)
            if forceConnection == 0 or force_find:
                infinite_loop = False
                distanceinclSlope, _, _ = distanceCalc3d(pZero, pOne)  # calculate 3d distance
                size = sa_f[1]  # size is in m3

                # Get closest unweighted wwtp
                sa_de_dist = distanceinclSlope - closestDistance
                if (np.exp(-sa_de_dist / closestDistance / sa_t) > random.random()):
                    selected_WWTP, selected_Coordinates, selected_Distance = idWWTPONE, pOne, distanceinclSlope

                # returns the node with the hightes potential
                connectivityIndex = distanceinclSlope * size ** (-1 * f_merge)
                sa_de_index = potIndex - connectivityIndex
                if (np.exp(-sa_de_index / potIndex / sa_t) > random.random()):
                    selected_Node, selected_potentialNodeCoordinates, selected_Index = idWWTPONE, pOne, connectivityIndex

    # 若存在死循环，则必须找到连接
    if infinite_loop:
        selected_Node, selected_potentialNodeCoordinates, selected_WWTP, selected_Coordinates = connectivityPotential(
            nodes, WWTPs, idWWTP, pZero, f_merge, sa_t, force_find=True)
    ...
    if potentialNode == None:  # No Connection was found
        ...
        return selected_Node, selected_potentialNodeCoordinates, selected_WWTP, selected_Coordinates
...
def updatePathLong(nodes, nodes_invert, path, sewersNoC, toNode):
    ...
    # Flows in Node
    flowInNode = nodes[path[0]][4]

    startFlow = flowInNode - flowAgainstDirection
    nodes_invert[path[0]][4] = startFlow
    ...
    for entry in path:
        ...
        if counter > 2 and finishReadingPath == 0:
            ...
            for i_index in nodes_invert:
                i = nodes_invert[i_index]
                    ...
                    if newest[0] == toNode:  # Last 3 nodes
                        if secondNewest[8] == 0:  # Arrived at the end of path or only three entries
                            if secondNewest[4] == 0:  # Connection to archPoint. Take over flow from node below
                                ...
                                nodes_invert[i_index][4] = updatedFlowSecondLast  # change second last node
                            ...
                            if secondNewest[4] == 0:  # no flow
                                ...
                                if third[0] == path[0]:
                                    ...
                                    if newest[0] not in sewersNoC:
                                        # If neweswt not in sewers, then the new branch is reached. Take over all flow
                                        notAgainstFlow = second[4]
                                    else:
                                        notAgainstFlow = second[4] - (newest[4] + newest[8])
...
def updatePathShort(nodes_Invert, pathtonearestWTPInvert):
    ...
    #         break
    s = nodes_Invert[pathtonearestWTPInvert[0]]
    flowFromNodeSwap = s[4] + s[8]
    ...
    #         break
    d = nodes_Invert[pathtonearestWTPInvert[1]]
    nodes_Invert[pathtonearestWTPInvert[1]][4] = d[4] + flowFromNodeSwap
...
def sumCostOfArchWWTPs(allPopNodesOntheWay, nodes, coefficients,
                       fc_coefficient_a, fc_coefficient_b):
    """
    This function sums the costs of all wwtp on the archpath.
    :param allPopNodesOntheWay: All popluated nodes on the way
    :param nodes: Nodes
    :param coefficient_a: coefficient of WWTP scale effect
    :param coefficient_b: coefficient of WWTP scale effect
    :param fc_coefficient_a: coefficient of WWTP sensitive analyze
    :param fc_coefficient_b: coefficient of WWTP sensitive analyze
    :return:
    ...
    for decentralizedWTP in allPopNodesOntheWay:  # Last one is not considered as it is calculated just above
        ...
        #         break
        i = nodes[decentralizedWTP]
        flowWTPonTheWay, _ = costWWTP(
            flow=(i[8] + i[4]),
            coefficients=coefficients,
            fc_coefficient_a=fc_coefficient_a,
            fc_coefficient_b=fc_coefficient_b
        )
        summedCostsWWPTS += flowWTPonTheWay
...
def costComparison(totCostI, totCostII, totCostIII, resonableCosts, costConnection, ignoreRC, forceCentralConnection,
                   finishedMerging, sa_t, expansion_module):
    """
    ...
    """
    # in expansion module, if totCostII is not the best choice, make its possibility lower
    if expansion_module and totCostII != min([totCostI, totCostII, totCostIII]):
        totCostII = max([totCostI, totCostII, totCostIII])

    # If a central connection is forced:
    if forceCentralConnection == True and finishedMerging:
        cost_dict = {totCostI: (1, 0),
                     totCostIII: (1, 1), }
        min_cost = min(cost_dict)
        max_cost = max(cost_dict)
        if (
                (sa_t < 0.00001) or
                (not np.isfinite(max_cost - min_cost)) or
                ((max_cost - min_cost) == 0)
        ):
            decCrit, wwtpSWAP = cost_dict[min_cost]
        else:
            for cost_temp in sorted(cost_dict.keys(), reverse=True):
                sa_de = (cost_temp - min_cost)/(max_cost - min_cost)
                if np.exp(-sa_de / sa_t) > random.random():
                    decCrit, wwtpSWAP = cost_dict[cost_temp]
                    break
    else:
        ...
        if ignoreRC != 1:
            if costConnection < resonableCosts:
                ...
                # totCostII has the lowest priority
                if totCostII == totCostI or totCostII == totCostIII:
                    totCostII = float("inf")
                # build the cost and plan diction
                cost_dict = {totCostI: (1, 0),
                             totCostII: (0, 0),
                             totCostIII: (1, 1), }
                min_cost = min(cost_dict)
                ...
                if (
                    ...
                    # explore the cost list from highest to lowest, to activate simulated annealing
                    for cost_temp in sorted(cost_dict.keys(), reverse=True):
                        sa_de = (cost_temp - min_cost)/(max_cost - min_cost)
                        if np.exp(-sa_de / sa_t) > random.random():
                            decCrit, wwtpSWAP = cost_dict[cost_temp]
            ...
            # totCostII has the lowest priority, when totCostII equals other cost, use the other plan
            if totCostII == totCostI or totCostII == totCostIII:
                totCostII = float("inf")
            # build the cost and plan diction
            cost_dict = {totCostI: (1, 0),
            ...
            if (
                ...
                for cost_temp in sorted(cost_dict.keys(), reverse=True):
                    ...
                    if np.exp(-sa_de / sa_t) > random.random():
                        decCrit, wwtpSWAP = cost_dict[cost_temp]
                        break
    # print(f"expansion_module:{expansion_module},"
    #       f"judge: {np.exp(-sa_de / min_cost / sa_t)},"
    #       f"delta: {sa_de/min_cost}")
...
def changeTD(nodes, edgesList, pumps, pathToNetwork, maxTD, minSlope, inflowNodes, sewers, minTD):
    ...
    for i in pathToNetwork:  # iterate over path and change trechdepth if needed up to maximum trench depth
        ...
        # if trenchFrom - minRequiredHDiff >= hTo - maxTD:
        Qtemp = nodes[toID][4] + nodes[toID][8]
        Q0 = 29095  # [m3/d], similar to 1e5[m3/s]
        lam = 4
        # print(f"Dorig, {(trenchFrom - minRequiredHDiff) - (hTo - maxTD):.2f}, "
        #       f"Qtemp, {Qtemp:.2f}, "
        #       f"lam, {lam * (Q0 / (Qtemp + Q0)):.2f} ,"
        #       f"judge, {((trenchFrom - minRequiredHDiff) - (hTo - maxTD)) +  lam * (Q0 / (Qtemp + Q0))<=0}")
        if ((trenchFrom - minRequiredHDiff) - (hTo - maxTD)) + lam * (Q0 / (Qtemp + Q0)) > 0:
        # if trenchFrom - minRequiredHDiff > hTo - maxTD:
            # Check if slope is steep enough and possible trench depth not too deep, use the average trench
            pumpFlow, lastID = False, pathToNetwork[-1][1][0]
            nodesCopy, pumps, liftHeight = checkTrenchLifting(
                lastID=lastID, sewers=sewers, pumps=pumps, nodesCopy=nodesCopy, nodes=nodes, trenchFrom=trenchFrom,
                trenchTo=trenchTo, minhDiffRequired=minRequiredHDiff, posToNode=positionToNode, inflowNodes=inflowNodes,
                toID=toID, fromID=fromID, minSlope=minSlope, pumpFlow=pumpFlow, hTo=hTo, hFrom=hFrom, minTD=minTD,
                maxTD=maxTD
            )
        else:
...
def correctPump(nodes, edgeList, pumps):
    nodesCopy = fastCopyNodes(nodes)
    edgesCopy = fastCopy(edgeList)
    pumpsCopy = fastCopy(pumps)

    # 用于记录可以出现泵站的ID
    pumpIdChecked = set()

    for SewerTemp in edgesCopy:  # 对每个管段进行检验
        # 获取起止点id、长度、埋深
        fromID, toID, length, slope = SewerTemp[0][0], SewerTemp[1][0], SewerTemp[2], SewerTemp[3]
        # 获取点位位置
        positionFromNode, _, _ = getTrenchDepth(nodesCopy, fromID)
        positionToNode, _, _ = getTrenchDepth(nodesCopy, toID)
        # 获取起止点埋深
        zFromNode = nodesCopy[positionFromNode][10]
        zToNode = nodesCopy[positionToNode][10]
        # 判断理论埋深是否等于实际深度，容差0.1m
        zNodeJudge = ((zFromNode - slope*length > zToNode - 0.1) and (zFromNode - slope*length < zToNode + 0.1)) or\
                     ((zFromNode + slope*length > zToNode - 0.1) and (zFromNode + slope*length < zToNode + 0.1))
        # 若两者不相等，则此处可以出现泵站。
        if not zNodeJudge:
            pumpIdChecked.add(fromID)
            pumpIdChecked.add(toID)

    # 对每个泵站进行检查
    for positionPump, pumpTemp in enumerate(pumpsCopy):
        # 若该泵站不在列表中，表示不应设置泵站
        if pumpTemp[0] not in pumpIdChecked:
            # 删除该泵站
            pumpsCopy.pop(positionPump)
...
def costPump(pathBetweenWWTPs, pumps, coefficients):  # Calculate pump costs
    """
    ...
    GridEmiFactor             --    grid emission factor
    ...
    """
    pumpCosts, summingPumpCosts = 0, 0

    for i in pathBetweenWWTPs:
        for pmp in pumps:
            if pmp[0] == i:
                flow, heightDifference = pmp[1], pmp[2]
                summingPumpCosts, _ = getPumpCostsDependingOnFlow(
                    Q=flow,
...
def correctTD(nodes, path, minTD, listwtps, maxTD, minSlope, sewers):
    ...
    # for pnt_temp in pnts:
    #     if pnt_temp[0] == path[0]:
    pnt_temp = pnts[path[0]]
    flowupStream = pnt_temp[4] + pnt_temp[8]  # initial flowupStream
    ...
    # Check if the initial node is a wwpt with inflowing nodes.
    # If it is a wwtp with inflow from other edges, change the trenchdepth to minium possible depth
    if pathlength == True:  # If more than one node
        for toWTPflowwingNode in sewers:  # Get all nodes flowing to this wwtp:
            if sewers[toWTPflowwingNode][0] == path[0] and toWTPflowwingNode != path[1]:
                ...
                #         hFlowingToNode = s[10]
                #         break
                s = nodes[toWTPflowwingNode]
                hFlowingToNode = s[10]

                # for s in nodes:
                #     if s[0] == path[0]:
                #         hNode, krit = s[10], pnt_temp[3] - pnt_temp[10]
                #         break
                s = nodes[path[0]]
                hNode, krit = s[10], pnt_temp[3] - pnt_temp[10]
                ...
                if krit != minTD and hFlowingToNode > neededTrenchDepth and neededTrenchDepth < (
                    ...
                    if toWTPflowwingNode in nodes:
                        if neededTrenchDepth < iterTrench:
                            trenchChange = 1
                            if neededTrenchDepth > hNode - minTD:
                                newTD = pnt_temp[3] - minTD
                                iterTrench = newTD
                            else:
                                newTD = neededTrenchDepth
                                iterTrench = newTD
                        break
                    # Check if toflowing node needs to be pumped.
                    # If so, don't changed heigth! (trenchdepth of toflowing node is deeper than trenchheightto
                    # for n in nodes:
        ...
        for toWTPflowwingNode in sewers:
            if sewers[toWTPflowwingNode][0] == path[0]:
                ...
                if krit != minTD and hFlowingToNode > neededTrenchDepth and neededTrenchDepth < (
                    ...
                    #         break
    if trenchChange == 1:
        pnts[path[0]][10] = newTD  # Don't change depth
    else:
        pnts[path[0]][10] = pnt_temp[3] - minTD  # Change depth
    ...
    for entry in path[1:]:
        pnt_temp = pnts[entry]
        if flowupStream != pnt_temp[4]:  # If flow in current node is not equal to flow in node above
            inflowNodes.append(entry)  # Add node to a list with all inflowing nodes. Don't change trench depth
        else:
            pnts[entry][10] = pnt_temp[3] - minTD  # Change trench depth to minium trench depth
        flowupStream = pnt_temp[4] + pnt_temp[8]  # Flow upstream in order to compare flow in node down the stream
        ...
        #             inflowNodes.append(entry)  # Add node to a list with all inflowing nodes. Don't change trench depth
        #         else:
        #             pnt_temp[10] = pnt_temp[3] - minTD  # Change trench depth to minium trench depth
        #         flowupStream = pnt_temp[4] + pnt_temp[8]  # Flow upstream in order to compare flow in node down the stream
        #         break
    return pnts, inflowNodes
...
def checkTrenchLifting(lastID, sewers, pumps, nodesCopy, nodes, trenchFrom, trenchTo, minhDiffRequired, posToNode,
    ...
    if toID in inflowNodes or toID == lastID:  # Node has inflowing nodes
        ...
        if liftTrenchDepth == 1:
            if pumpFlow == True:  # If trench depth would be too deep
                ...
                newHeightDiff = nodesCopy[posToNode][10] - trenchFrom + minhDiffRequired
                # Calculate height difference which needs to be pumped
                if newHeightDiff <= 0:
                    newHeightDiff = nodesCopy[posToNode][10] - ZToDepth + minhDiffRequired
            ...
            if (
                    pumpFlow and
                    (ZToDepth < hTo - maxTD) and
                    ((nodesCopy[posToNode][10] - trenchFrom + minhDiffRequired > 0) or
                     (nodesCopy[posToNode][10] - ZToDepth + minhDiffRequired > 0))
                ):  # As trench lift is not possible, depth is not changed
            # if ZToDepth < hTo - maxTD:  # As trench lift is not possible, depth is not changed
                # As trench lift is not possible, depth is not changed, use the average trench
                newHeightDiff = nodesCopy[posToNode][10] - trenchFrom + minhDiffRequired
                # Calculate height difference which needs to be pumped
                if newHeightDiff <= 0:  # trenchFrom is higher than nodesCopy[posToNode][10]. Might occur because similar heights
                    newHeightDiff = nodesCopy[posToNode][10] - ZToDepth + minhDiffRequired
                    # Even though there is a slope, not steep enough. Pump only the height it would need for free flow
                pumps = addPump(pumps, fromID, newHeightDiff, nodes)  # Add pump
            else:  # free flow is possible and trench depth within limits
        ...
        if pumpFlow == True:  # has no inflowing node. Free flow is not possible
            ...
            newHeightDiff = nodesCopy[posToNode][10] - trenchFrom + minhDiffRequired
            # Calculate height difference which needs to be
            if newHeightDiff <= 0:
                newHeightDiff = nodesCopy[posToNode][10] - ZToDepth + minhDiffRequired
            pumps = addPump(pumps, fromID, newHeightDiff, nodes)  # Add pump
        else:
    ...
    return nodesCopy, pumps, newHeightDiff
...
def sortPN(PN, num):
    PN_end = min(num, len(PN))
    PN_temp_1 = copy.copy(PN[0:PN_end])
    PN_temp_2 = copy.copy(PN[PN_end:])
    PN_temp_1 = sorted(PN_temp_1, key=(lambda x: x[0]))
    PN_sorted = PN_temp_1 + PN_temp_2
    return PN_sorted
...
def primCalculations(nodes, PN, allPopNodesOntheWay):
    ...
    """
    # 仅对前300个进行排序
    PN_sorted = sortPN(PN, 300)

    # 仅更新前200个
    PN_end = min(200, len(PN_sorted))
    for a in allPopNodesOntheWay:
        idp1, p1, _, _, _, _ = getPns(a, nodes)
        for i in PN_sorted[0: PN_end]:
    ...
    # 由于仅更新了前200个，所以仅需对前200个进行排序
    PN_sorted = sortPN(PN_sorted, 200)
    return PN_sorted
...
def checkIfWWTPwereConnected(path, wwtpsIterate, WWTPs):
    """
    This functions checks if by connecting a wwtp another wwtp was connected or not. If yes, this wwtp gets deleted from list of wwtps.
...
def assignInitialFlow(nodes, path, initialFlow, position):
    ...
    #         break
    i = nodes[path[position]]
    nodes[path[position]][4] = initialFlow - i[8]
...
def getFullHypotheticalCosts(aggregatetPoints, WWTPs, sewers, pumps, coefficients,
                             edgeList, nodes, stricklerC,
                             f_SewerCost, fc_coefficient_a, fc_coefficient_b):
    """
    This function calculates total system systems and assumes decentralized solution for all not yet considered nodes.
    :param aggregatetPoints: All nodes to consider
    :param WWTPs: Waste water treatment plants
    :param sewers: Sewer Network
    :param pumps: List with pumps
    :param coefficients: coefficients of GHG and cost calculating
    :param edgeList: List with edges
    :param nodes: List with nodes
    :param stricklerC: Strickler coefficient
    :param f_SewerCost: cost factor sewer
    :param fc_coefficient_a: cost factor coefficient a
    :param fc_coefficient_b: cost factor coefficient b
    :return:
    degCen              -    Z
    ...
    (completePumpGhg, completeWWTPGhg, completePipeGhg,
     completePumpCosts, completeWWTPCosts, completePipeCosts) = calculatetotalAnnuities(
        listWTPs=hypotheticalWWTP,
        pumps=pumps,
        coefficients=coefficients,
        sewers=sewers,
        edgeList=edgeList,
        nodes=nodes,
        stricklerC=stricklerC,
        fc_SewerCost=f_SewerCost,
        fc_coefficient_a=fc_coefficient_a,
        fc_coefficient_b=fc_coefficient_b
    )

    fullGhg = completePumpGhg + completeWWTPGhg + completePipeGhg
    fullCosts = completePumpCosts + completeWWTPCosts + completePipeCosts
    return (degCen, degCenWeighted, fullGhg, completePumpGhg, completeWWTPGhg, completePipeGhg, listWWTPwithAggregatedNodes,
            [fullCosts, completePumpCosts, completeWWTPCosts, completePipeCosts, sumFlow, sinks])
...
def SNIP(OnlyExecuteMerge, outListFolder, runNr, nodes, anteilDaten, streetNetwork, startnode, edgeList, streetVertices,
         ...
         z_to_reach, sa_t_ori_e, sa_alpha_ori_e, sa_t_ori_m, sa_alpha_ori_m, sa_co_vary):
    """
    SNIP Algorithm
    ...
    completePumpGhg, completeWWTPGhg, completePipeGhg, totalSystemCosts    -    costs
    """
    # simulated annealing parameters
    sa_t_e = copy.deepcopy(sa_t_ori_e)
    sa_alpha = copy.deepcopy(sa_alpha_ori_e)

    # Input parameters
    minTD = inParameter["minTD"]
    # [meter] Min trench depth
    maxTD = inParameter["maxTD"]
    # [meter] Maximum trench depth
    minSlope = inParameter["minSlope"]
    # [%] Criteria of minimum slope without pumps needed
    f_merge = inParameter["f_merge"]
    # Factor do determine how the OST are reconnected. Puts the size in relation ot the distance. If large, the size gets more important
    neighborhood = inParameter["neighborhood"]
    # How large the neibhourhood is for searching paths
    streetFactor = inParameter["f_street"]
    # How much the path follows the roads
    reasonableGHGPerM3 = inParameter["reasonableGHGPerM3"]
    # reasonable GHG emissions Per m3 wastewater
    stricklerC = inParameter["stricklerC"]
    # [m3/s] Strickler coefficient
    city = inParameter["city"]
    # the name of city
    coefficients = inParameter["coefficients"]

    print(" ")
    print("All Paramterers")
    print("----------------")
    ...
    print("GridEmiFactor: " + str(coefficients["GridEmiFactor"]))
    print("stricklerC: " + str(stricklerC))
    print("EW_Q: " + str(coefficients["EW_Q"]))
    print("WWTP emissions: ")
    print("    C={:.2f} * Q^{:.2f}".format(coefficients["coefficient_a"], coefficients["coefficient_b"]))
    print(" ")
    ...
    f_topo = inParameter["f_topo"]
    # [-] Weighting factor for the DEM graph creation of the topography
    f_SewerCost = inParameter["fc_SewerCost"]
    # [-] cost factor
    fc_coefficient_a = inParameter["fc_coefficient_a"]
    # [-] cost factor
    fc_coefficient_b = inParameter["fc_coefficient_b"]
    # [-] cost factor

    # Initial calculations
    hypoZWeighted = 0  # Initial weighted Z value
    hypoTotalGhgOld = 0  # Initialization Hypothetical costs
    ...
    ZtoReach = z_to_reach  # In case artifically connection is enforced SNIP terminates at this value (must be smaller than 1)
    ...
    progress_record(txtResultPath=txtResultPath,
                    name="speed_record",
                    process_name="步骤名称",
                    progress="权重",
                    progress_all="目标权重",
                    sec_per_item="速度s/item",
                    sa_t="温度",
                    first_ite=True)
    total_system_costs_record(txtResultPath=txtResultPath,
                              name="totalSystemCosts_record",
                              hypoZ="集中度", hypoZWeighted="加权集中度",
                              hypoTotalGhg="总碳排放", hypoPumpGhg="泵站碳排放",
                              hypoWWTPGhg="水厂碳排放", hypoPipeGhg="管网碳排放",
                              hypoTotalCosts="总成本", hypoPumpCosts="泵站成本",
                              hypoWWTPcosts="水厂成本", hypoPipeCosts="管网成本",
                              sumFlow="总处理水量", WWTP_number="水厂数量",
                              first_ite=True)
    progress_text_old = None

    # Expansion Module is activated
    while expansion == 1:
        with open("{}/log_{}.txt".format(r"./log", city[0:3]), "a") as f:
            print("\nStart expansion module...", file=f)
        ...
        if OnlyExecuteMerge == 0 and ignoreOnlyExecuteMerge == 0:
            while len(PN) > 0 or firstIteration == 1:
                if ZtoReach > hypoZWeighted:  # Used to abort in case a certain z-value is reached
                    PN, FROMNODE, TONODE, weightFactorDijkstra, realDistance = getClosestNode(PN)
                    # Select next node to connect
                    ...
                    if TONODE not in sewers:
                        ...
                        if iterativeCostCalc == 1 and runNr == 1:
                            (hypoZ, hypoZWeighted, hypoTotalGhg, hypoPumpGhg, hypoWWTPGhg,
                             hypoPipeGhg, hypoWWTPcorrectFlow, hypoCosts) = getFullHypotheticalCosts(
                                aggregatetPoints=aggregatetPoints,
                                WWTPs=WWTPs,
                                sewers=sewers,
                                pumps=pumps,
                                coefficients=coefficients,
                                ...
                                nodes=nodes,
                                stricklerC=stricklerC,
                                f_SewerCost=f_SewerCost,
                                fc_coefficient_a=fc_coefficient_a,
                                fc_coefficient_b=fc_coefficient_b
                            )

                            if reActivationEM == 0:
                                if hypoTotalGhg < hypoTotalGhgOld:
                                    sa_t_e *= sa_alpha
                                    totalSystemCosts.append(
                                        [hypoZ, hypoZWeighted, hypoTotalGhg, hypoPumpGhg, hypoWWTPGhg, hypoPipeGhg,
                                         hypoCosts[0], hypoCosts[1], hypoCosts[2], hypoCosts[3], hypoCosts[4], hypoCosts[5]])  # Store costs in list

                                    # 记录一次迭代的时间
                                    time_now, pass_time, end_mes = calculate_time(time_now)
                                    # progress bar
                                    progress_text_old = progress_bar(
                                        process_name="{}_Expansion (PN-{:>4})".format(city, len(PN)),
                                        progress=hypoZWeighted,
                                        progress_all=ZtoReach,
                                        end_mes=end_mes,
                                        progress_text_old=progress_text_old)
                                    progress_record(txtResultPath=txtResultPath,
                                                    name="speed_record",
                                                    process_name="Expansion",
                                                    progress=hypoZWeighted,
                                                    progress_all=ZtoReach,
                                                    sa_t=sa_t_e,
                                                    sec_per_item=pass_time)
                                    # 实时记录到文本
                                    total_system_costs_record(txtResultPath=txtResultPath,
                                                              name="totalSystemCosts_record",
                                                              hypoZ=hypoZ, hypoZWeighted=hypoZWeighted,
                                                              hypoTotalGhg=hypoTotalGhg, hypoPumpGhg=hypoPumpGhg,
                                                              hypoWWTPGhg=hypoWWTPGhg, hypoPipeGhg=hypoPipeGhg,
                                                              hypoTotalCosts=hypoCosts[0], hypoPumpCosts=hypoCosts[1],
                                                              hypoWWTPcosts=hypoCosts[2], hypoPipeCosts=hypoCosts[3],
                                                              sumFlow=hypoCosts[4], WWTP_number=hypoCosts[5],)


                            hypoTotalGhgOld = hypoTotalGhg
                        ...
                        if firstIteration == 0:
                            ...
                            try:  # Try to find path on street with Dijkstra Algorithm
                                ...
                                streetConnection = 1                                    # criteria whether the djikstra distance or MST distance was
                            except KeyError:
                                streetConnection = 0
                        ...
                        if firstIteration == 0:
                            ...
                            # pumpsA1 = correctPump(nodesA1, edgeListI, pumpsA1)  # delete useless Pumps
                            pipeCostA1, edgeListI = costToWTP(
                                pathToWTP=pathtonearestWTP,
                                edgesID=edgeListI,
                                nodes=nodesA1,
                                pumps=pumpsA1,
                                minTD=minTD,
                            ...
                            WWTPcostsA1, _ = costWWTP(
                                flow=(flowWWFrom + flowAllArchPathWWTPs),
                                coefficients=coefficients,
                                fc_coefficient_a=fc_coefficient_a,
                                fc_coefficient_b=fc_coefficient_b
                            )
                            # Costs of new WWTPs with original Flow, flow of connecting node and flow of all wwtps on archPaths

                            # Option 1 - Pumping costs
                            pumpCostA1 = costPump(
                                pathBetweenWWTPs=pathtonearestWTP,
                                pumps=pumpsA1,
                                coefficients=coefficients
                            )  # Calculate annual pumping costs
                            ...
                            #         flowTo = fl[8] + fl[4]
                            #         break
                            fl = nodes[pathNearWTPInvert[-1][0]]
                            flowTo = fl[8] + fl[4]
                            ...
                            #         flowFrom = fl[8] + fl[4]
                            #         break
                            fl = nodes[pathNearWTPInvert[0][0]]
                            flowFrom = fl[8] + fl[4]
                            ...
                            costDecentralWWTPs = sumCostOfArchWWTPs(
                                allPopNodesOntheWay=allPopNodesOntheWay[:-1],
                                nodes=nodes,
                                coefficients=coefficients,
                                fc_coefficient_a=fc_coefficient_a,
                                fc_coefficient_b=fc_coefficient_b
                            )  # Calculate costs of new wwtps on added path
                            # cost of already build wwwt
                            wtpCostClosestARA, _ = costWWTP(
                                flow=flowWWFrom,
                                coefficients=coefficients,
                                fc_coefficient_a=fc_coefficient_a,
                                fc_coefficient_b=fc_coefficient_b
                            ...
                            pipeCostA2, _ = costToWTP(
                                pathToWTP=pathSubNetworkToClosestWWTP,
                                edgesID=edgeList,
                                nodes=nodes,
                                pumps=pumps,
                                minTD=minTD,
                            ...
                            pumpCostA2 = costPump(
                                pathBetweenWWTPs=pathSubNetworkToClosestWWTP,
                                pumps=pumps,
                                coefficients=coefficients
                            )# Pumping costs II
                            ...
                            # pumpsA3 = correctPump(nodesA3, edgeListIII, pumpsA3)  # delete useless Pumps
                            # Option 3 - Pumping costs
                            pumpCostA3 = costPump(
                                pathBetweenWWTPs=pathtonearestWTPInvert,
                                pumps=pumpsA3,
                                coefficients=coefficients
                            )  # Pump Costs III

                            # Calculate costs
                            pipeCostA3, edgeListIII = costToWTP(
                                pathToWTP=pathtonearestWTPInvert,
                                edgesID=edgeListIII,
                                nodes=nodesA3,
                                pumps=pumpsA3,
                                minTD=minTD,
                                toNode=closestARAtraditionell,
                                stricklerC=stricklerC,
                                f_SewerCost=f_SewerCost,
                                coefficients=coefficients
                            )  # sum all, use negative slopes
                            ...
                            costConnectionA = calculateConnectionCosts(pipeCostA1, pumpCostA1,
                                                                       pipeCostA2, pumpCostA1,
                                                                       pipeCostA3, pumpCostA1,
                                                                       WWTPcostsA1)

                            resonableCosts = summedFlowDecentralWWTP / reasonableGHGPerM3  # Total reasonable costs
                            # Calculate reasonable costs (If positive: central costs are more expensive, if negative: decentral is more expensive)                                                                                                       # 1: Ignore, 0: Don't Ignore

                            swapCriteria, dezentralCriteria = costComparison(
                                totCostI=totCostsA1,
                                totCostII=totCostsA2,
                                totCostIII=totCostsA3,
                                resonableCosts=resonableCosts,
                                costConnection=costConnectionA,
                                ignoreRC=0,
                                forceCentralConnection=forceConnection,
                                finishedMerging=True,
                                sa_t=sa_t_e,
                                expansion_module=True
                            )
                            # Compare costs and select option, consider reasonable costs
        ...
        if Zreached == 0:  # Go to Merging Module
            if sa_co_vary:
                sa_t_ori_m = sa_t_e
            sa_t_temp = copy.deepcopy(sa_t_e)
            nodes, sewers, pumps, WWTPs, PN, edgeList, totalSystemCosts, progress_text_old, sa_t_e = mergingModule(
                OnlyExecuteMerge=OnlyExecuteMerge, forceZ=forceZ, ZtoReach=ZtoReach, hypoZWeighted=hypoZWeighted,
                aggregatetPoints=aggregatetPoints, nodes=nodes, WWTPs=WWTPs, sewers_Current=sewers_Current,
                edgeList=edgeList, pumps=pumps, PN=PN, sewers=sewers, streetNetwork=streetNetwork, f_merge=f_merge,
                minTD=minTD, maxTD=maxTD, minSlope=minSlope, stricklerC=stricklerC, coefficients=coefficients,
                f_SewerCost=f_SewerCost, fc_coefficient_a=fc_coefficient_a, fc_coefficient_b=fc_coefficient_b,
                totalSystemCosts=totalSystemCosts, iterativeCostCalc=iterativeCostCalc, txtResultPath=txtResultPath,
                city=city, progress_text_old=progress_text_old, sa_t_ori_m=sa_t_ori_m, sa_alpha_ori_m=sa_alpha_ori_m
            )
    ...
    print("\n\nSNIP is successfully calculated.")
    MergeTime, ExpansionTime = 0, 0

    # writeTotxt(txtResultPath, "totalSystemCosts", totalSystemCosts)  # Append costs to statistics.
    # myDocument = open(txtResultPath + "totalSystemCosts.txt", 'r+')
    # content = myDocument.read()
    # new_content = content.replace("[", "")
    # new_content = new_content.replace("]", "")
    # new_content = "集中度,加权集中度,总花费,泵站花费,水厂花费,管道花费\n" + new_content
    # myDocument.seek(0)
    # myDocument.write(new_content)
    # myDocument.truncate()
    # myDocument.close()
    # os.rename(txtResultPath + "totalSystemCosts.txt", txtResultPath + "totalSystemCosts.csv")

    return (ExpansionTime, MergeTime, final_Network, flowPoints, WWTPs, final_wwtps, final_Pumps, edgeList,
            completePumpGhg, completeWWTPGhg, completePipeGhg, totalSystemCosts, buildings, buildPoints, aggregatetPoints,
            [completePumpCosts, completeWWTPCosts, completePipeCosts])


def mergingModule(OnlyExecuteMerge, forceZ, ZtoReach, hypoZWeighted, aggregatetPoints, nodes, WWTPs,
                  sewers_Current, edgeList, pumps, PN, sewers, streetNetwork, f_merge, minTD, maxTD,
                  minSlope, stricklerC, coefficients,
                  f_SewerCost, fc_coefficient_a, fc_coefficient_b, totalSystemCosts, iterativeCostCalc,
                  txtResultPath, city, progress_text_old, sa_t_ori_m, sa_alpha_ori_m):
    '''
    Merging Module
    ...
    with open("{}/log_{}.txt".format(r"./log", city[0:3]), "a") as f:
        print("\nStart merging module...", file=f)
    time_now = time.time()

    # # 定义模拟退火算法的温度
    sa_t_m = sa_t_ori_m
    # # 定义模拟退火算法的降温速度
    sa_alpha_m = sa_alpha_ori_m
    ...
    if iterativeCostCalc == 1:
        ...
        hypoZWeightedOld = hypoZOld
    else:
        # hypoZOld = 0
        hypoZWeightedOld = 0

    # As long not merging is exited and there are wwtps to reconnect
    # =====len(WWTPs)>2 was changed to len(WWTPs)>1
    while len(sortedListWWTPs) > 1 and ZtoReach > hypoZWeighted and len(WWTPs) > 2 and sa_t_m > 1:  # last is new

        wwtpsIterate, expansionM = fastCopy(WWTPs), True

        # In case a connection is forced
        if (forceZ and len(sortedListWWTPs) == 2 or finishedExpansion) and mergeCostStorage != []:
            ...
            if ZtoReach > hypoZWeighted:
                ...
                if len(sortedListWWTPs) == 2 or finishedExpansion:
                    mergeCostStorage, cheapestM = getCheapestMerge(mergeCostStorage, WWTPs, sa_t_m)  # get cheapest connection
                    CurrentCheckedWWTP, checkBackConnectionID = cheapestM[1], cheapestM[2]

                else:
                    CurrentCheckedWWTP, checkBackConnectionID = sortedListWWTPs[0], sortedListWWTPs[0][0]
                    # Current WWTP to check for merge, Get largest wwtp to check
        ...
        while len(wwtpsIterate) > 1 and expansionM == True and sa_t_m > 1:  # Iterate until all WWTPs are checked
            ...
            wwtpToWwtpConncetion = 0  # Criteria to define wheter wwtps are connected or not
            ...
            while expansionM == True:
                ...
                nodeIdOpt1, nodeOpt1Cor, nodeIdOpt3, nodeOpt3Cor = connectivityPotential(nodes, WWTPs,
                                                                                         ...
                                                                                         f_merge, sa_t_m)  # Option 1 & Option 3
                # print(nodeIdOpt1, nodeOpt1Cor, nodeIdOpt3, nodeOpt3Cor)
                ...
                # print(nodeIdOpt2, nodeOpt2Cor, pathInClosestNetwork)
                ...
                if ZtoReach > hypoZWeighted and finishedExpansion:  #
                    mergeOptions = [cheapestM[3]]

                # delete the repeated node
                for num, mOpt in enumerate(mergeOptions):
                    if mOpt[0] == checkBackConnectionID:
                        del mergeOptions[num]
                ...
                if expansionM == True:
                    ...
                    while iterateMergeOptions == True:
                        ...
                        for mOpt in mergeOptions:
                            ...
                            if len(nodesToNetwork) > 1:
                                # Only the inFlow from other nodes on the path gets calculated
                                nodes_BI = getInflowingNodes(nodes_BI, nodesToNetwork,
                                                             flowInitial_to, notInaNetworkBeforeConnection)
                            else:
                                # Change flow in nodes of last element
                                nodes_BI = assignInitialFlow(nodes_BI, pathBetweenWWTPs, flowInitial_to, 0)

                            # Clear flow where not in starting or ending network
                            nodes_BI = clearFlow(nodes_BI, notInaNetwork)
                            # Change Flow along the path in nodes_BI
                            nodes_BI = changeFlowAlongPath(nodes_BI, pathBetweenWWTPs)
                            # Set all trench depth except inflow nodes to minimum trench depth
                            nodes_BI, inflowNodesWTPB1 = correctTD(nodes_BI, archPathWWTPInvert[:-1], minTD, WWTPs,
                                                                   maxTD, minSlope, sewers)
                            ...
                            # pumpWWTPListB1 = correctPump(nodes_BI, edgeList1, pumpWWTPListB1)  # delete useless Pumps
                            pipeCostsB1, edgeList1 = costsBetweenWWTPs(
                                pathToWTP=pathBetweenWWTPs,
                                edgesID=edgeList1,
                                nodes=nodes_BI,
                                inverse=0,
                                pumps=pumpWWTPListB1,
                                minTD=minTD,
                                stricklerC=stricklerC,
                                f_SewerCost=f_SewerCost,
                                coefficients=coefficients
                            )  # first zero: regular flow, second zero: proportional costs

                            # Pumping Costs
                            pumpCostB1 = costPump(
                                pathBetweenWWTPs=pathBetweenWWTPs,
                                pumps=pumpWWTPListB1,
                                coefficients=coefficients
                            )  # Calculate pump costs

                            # Option 1 - WWTPs costs
                            wtpCostB1, _ = costWWTP(
                                flow=(flowWWFrom + flowWWTO),
                                coefficients=coefficients,
                                fc_coefficient_a=fc_coefficient_a,
                                fc_coefficient_b=fc_coefficient_b
                            )  # WWTP-costs: Additional costs of building a larger WWTP
                            ...
                            if len(nodesFromNetwork) == 0 and len(nodesToNetwork) == 0:
                                ...
                                pipeCostB2a, _ = costsBetweenWWTPs(
                                    pathToWTP=nodesFromNetwork,
                                    edgesID=edgeList,
                                    nodes=nodes,
                                    inverse=0,
                                    pumps=pumps,
                                ...
                                pipeCostB2b, _ = costsBetweenWWTPs(
                                    pathToWTP=nodesToNetwork,
                                    edgesID=edgeList,
                                    nodes=nodes,
                                    inverse=0,
                                    pumps=pumps,
                                ...
                                pumpCostB2A = costPump(nodesFromNetwork, pumps,
                                                       coefficients)  # [CHF] Calculate pump costs
                                pumpCostB2B = costPump(nodesToNetwork, pumps,
                                                       coefficients)  # [CHF] Calculate pump costs
                                pumpCostB2 = pumpCostB2A + pumpCostB2B

                            # WWTP costs   
                            wtpCostB2a, _ = costWWTP(
                                flow=flowWWFrom,
                                coefficients=coefficients,
                                fc_coefficient_a=fc_coefficient_a,
                                fc_coefficient_b=fc_coefficient_b
                            )
                            wtpCostB2b, _ = costWWTP(
                                flow=flowWWTO,
                                coefficients=coefficients,
                                fc_coefficient_a=fc_coefficient_a,
                                fc_coefficient_b=fc_coefficient_b
                            ...
                            if needsNetworkRemoving == 1:  # Calculate costs of crossed WWTPs
                                sumCostcrossedWWTP = getCostsOfCrossedWWTPs(
                                    allNodesToAddToPN=allNodesToAddToPN,
                                    pathBetweenWWTPs=pathBetweenWWTPs,
                                    WWTPS_noCon=WWTPS_noCon,
                                    sewers_NoCon=sewers_NoCon,
                                    nodes_noCon=nodes_noCon,
                                    coefficients=coefficients,
                                    fc_coefficient_a=fc_coefficient_a,
                                    fc_coefficient_b=fc_coefficient_b,
                                )
                            ...
                            # pumpWWTPB3 = correctPump(nodes_B3, edgeList3, pumpWWTPB3)  # delete useless Pumps
                            # Pumping Costs
                            pumpCostB3 = costPump(pathBetweenWWTPsInvert, pumpWWTPB3,
                                                  coefficients)  # Calculate pump costs

                            # Sewer costs
                            pipeCostB3, edgeList3 = costsBetweenWWTPs(
                                pathToWTP=pathBetweenWWTPsInvert,
                                edgesID=edgeList3,
                                nodes=nodes_B3,
                                inverse=flowInitial_to,
                                pumps=pumpWWTPB3,
                                minTD=minTD,
                                stricklerC=stricklerC,
                                f_SewerCost=f_SewerCost,
                            ...
                            wwtpSWAP, wwtpToWwtpConncetion = costComparison(
                                totCostI=totCostBI,
                                totCostII=totCostB2,
                                totCostIII=totCostB3,
                                resonableCosts=0,
                                costConnection=0,
                                ignoreRC=1,
                                forceCentralConnection=forceZ,
                                finishedMerging=finishedExpansion,
                                sa_t=sa_t_m,
                                expansion_module=False
                            )  # Compare costs, don't consider reasonable  costs
                            ...
                            if wwtpToWwtpConncetion == 1:
                                WWTPs, wwtpsIterate = checkIfWWTPwereConnected(pathBetweenWWTPs, wwtpsIterate, WWTPs)
                                # Check if by a wwtp connection another wwpt was connected as well If yes, delete these
                                ...
                                if wwtpSWAP == 1:
                                    ...
                                    if iterativeCostCalc == 1 and ZtoReach > hypoZWeighted:
                                        (hypoZ, hypoZWeighted, hypoTotalGhg, hypoPumpGhg,
                                         hypoWWTPGhg, hypoPipeGhg, _, hypoCosts) = getFullHypotheticalCosts(
                                            aggregatetPoints=aggregatetPoints,
                                            WWTPs=WWTPs,
                                            sewers=sewers,
                                            pumps=pumps,
                                        ...
                                        # if hypoZ > hypoZOld:
                                        if hypoZWeighted > hypoZWeightedOld:
                                            sa_t_m *= sa_alpha_m
                                            totalSystemCosts.append(
                                                [hypoZ, hypoZWeighted, hypoTotalGhg, hypoPumpGhg, hypoWWTPGhg, hypoPipeGhg,
                                                 hypoCosts[0], hypoCosts[1], hypoCosts[2], hypoCosts[3],
                                                 hypoCosts[4], hypoCosts[5]])
                                            # 记录一次迭代的时间
                                            time_now, pass_time, end_mes = calculate_time(time_now)
                                            # process bar
                                            progress_text_old = progress_bar(
                                                process_name="{}_Merging (ListWWTP-{:>4}, WWTP-{:>4})".format(city, len(sortedListWWTPs), len(WWTPs)),
                                                progress=hypoZWeighted,
                                                progress_all=ZtoReach,
                                                end_mes=end_mes,
                                                progress_text_old=progress_text_old)
                                            ...
                                            total_system_costs_record(txtResultPath=txtResultPath,
                                                                      ...
                                                                      hypoTotalGhg=hypoTotalGhg, hypoPumpGhg=hypoPumpGhg,
                                                                      hypoWWTPGhg=hypoWWTPGhg, hypoPipeGhg=hypoPipeGhg,
                                                                      hypoTotalCosts=hypoCosts[0],
                                                                      hypoPumpCosts=hypoCosts[1],
                                                                      hypoWWTPcosts=hypoCosts[2],
                                                                      hypoPipeCosts=hypoCosts[3],
                                                                      sumFlow=hypoCosts[4], WWTP_number=hypoCosts[5],)
                                            # hypoZOld = hypoZ
                                            hypoZWeightedOld = hypoZWeighted
                                    ...
                                    if needsNetworkRemoving == 1:
                                        ...
                                        if PN == []:  # ALL deleted nodes were on path, don't quit expnasion module
                                            ...
                                            return nodes, sewers, pumps, WWTPs, PN, edgeList, totalSystemCosts, progress_text_old, sa_t_m  # Return to expansion-module
                                    break
                                else:
                                    sortedListWWTPs = delEntry(sortedListWWTPs,
    ...
    return nodes, sewers, pumps, WWTPs, PN, edgeList, totalSystemCosts, progress_text_old, sa_t_m


def getCheapestMerge(mergeCostStorage, WWTPs, sa_t):
    '''
    ...
    '''
    cheapestMerge = float("inf")

    # Iterate all merges to find the cheapest
    for e in mergeCostStorage:
        if e[0] < cheapestMerge:
            ok, ok2 = checkifInWWTPs(WWTPs, e[3][0]), checkifInWWTPs(WWTPs, e[2])
            if ok == True and ok2 == True and e[3][0] != e[2]:
                cheapestMerge = copy.copy(e[0])

    # Iterate all merges using Simulated Annealing
    for sa_e in mergeCostStorage:
        sa_de = sa_e[0] - cheapestMerge
        if (np.exp(-sa_de / cheapestMerge / sa_t) > random.random()):
            ok, ok2 = checkifInWWTPs(WWTPs, sa_e[3][0]), checkifInWWTPs(WWTPs, sa_e[2])
            if ok == True and ok2 == True and sa_e[3][0] != sa_e[2]:
                selected_M = sa_e
                Merge_selected = copy.copy(sa_e[0])
    ...
    for e in mergeCostStorage:
        ok, ok2 = checkifInWWTPs(WWTPs, e[3][0]), checkifInWWTPs(WWTPs, e[2])
        if ok == True and ok2 == True and e[3][0] != e[2]:
            if e[0] != Merge_selected:
                copyList.append(e)
    mergeCostStorage = copyList
    return mergeCostStorage, selected_M
    ...
    for nodeToCon in nodesUpdateCalc:
        ...
        if isPop > 0:
            if idp1 in network:
                ...
                for index in nodes:
                    i = nodes[index]
                    if index not in nodesUpdateCalc and index in network:
...
def dijkstra(streetNetwork, idp0, idp1, heightDiff):
    ...
    # print("idp0:{}; idp1:{}; distStartEnd:{}".format(idp0, idp1, distStartEnd))
...
def primResultGISList(DrawHouses, VerticGraph, pntsFlowMin, streetVertices, buildings, buildPoints, rasterPoints,
    ...
    for i in VerticGraph:  # Get flow, pt_from and pt_to and append to z_list
        ...
        if pt_from != () and pt_to != ():
            ...
            z = (0, [korX_from_pt, korY_from_pt], [korX_to_pt, korY_to_pt], flow, pipeDiameter, averageTD, pipeLength, pipeSlope)
...
def assignStreetVertAggregationMode(aggregatetPoints, streetVertices, minTD):
    ...
    # Add all streetVertices into forSNIP
    for vertex in streetVertices:
        xVertex, yVertex, zVertex = vertex[1], vertex[2], vertex[3]
        trenchDepth = zVertex - minTD
        forSNIP[vertex[0]] = [vertex[0], xVertex, yVertex, zVertex, 0, 0, 0, 0, 0, [], trenchDepth]

    # Add all agreggatet points into forSNIP with scrapID and replace if point already existing
    for point in aggregatetPoints:
        x1, y1 = point[1], point[2]
        # Check wheter the added point was in streetVertices
        for i in forSNIP:
            streetID, xNear, yNear, zNear = forSNIP[i][0], forSNIP[i][1], forSNIP[i][2], forSNIP[i][3]

            # Assign correct ID and FLOW
            if xNear == x1 and yNear == y1:
                point[0] = streetID
                forSNIP[streetID] = point
                forSNIP[streetID][3] = zNear  # Assign height
                forSNIP[streetID][10] = zNear - minTD
    # forSNIP = dictToList(forSNIP)  # Dictionary to list
...
def progress_bar(process_name, progress, progress_all, end_mes=None, progress_text_old=None):
    return "No progress_bar"
    # # 计算进度条信息
    # progress_length = int(progress / progress_all * 20)
    # # 生成末尾信息
    # if end_mes != None:
    #     end_mes = ", " + end_mes
    # # 进度条文字
    # progress_text = "{}_ ({}{})，{:>7.2%}, {}{}".format(
    #     process_name,
    #     "▋" * progress_length,
    #     " " * (20 - progress_length),
    ...
    # )
    # # 进度条文字处理
    # progress_text = progress_text.replace(":", "_")
    # progress_text = progress_text.replace("/", "_")
    # progress_text = progress_text.replace("%", "")
    # # 生成进度条文件夹
    # logFolder = r"./log"
    # if progress_text_old is not None:
    #     if os.path.exists("{}/{}".format(logFolder, progress_text_old)):
    #         os.rmdir("{}/{}".format(logFolder, progress_text_old))
    # # 以文件夹形式显示进度条
    # os.mkdir("{}/{}".format(logFolder, progress_text))
    #
    # # 将进度写入文本
    # if progress_text_old is None:
    #     write_mode = "w"
    # else:
    #     write_mode = "a"
    # with open("{}/log_{}.txt".format(logFolder, process_name[0:3]), write_mode) as f:
    #     print("\r{}: |{}{}|，{:>7.2%}, {}{}".format(
    #         process_name,
    #         "▋" * progress_length,
    #         " " * (20 - progress_length),
...
def calculate_time(time_now):
    # 记录一次迭代的时间
    time_old = copy.copy(time_now)
    time_now = time.time()
    pass_time = time_now - time_old
    if pass_time < 60:
        end_mes = "{:>4.2f}s/item".format(pass_time)
    else:
        end_mes = "{:>4.2f}min/item".format(pass_time / 60)
    return time_now, pass_time, end_mes

def progress_record(txtResultPath, name, process_name, progress, progress_all, sec_per_item, sa_t, first_ite=False):
    if first_ite:
        write_mode = "w"
    else:
        write_mode = "a"
    with open(txtResultPath + name + ".txt", write_mode) as f:
        f.write("{},{},{},{},{}\n".format(process_name, progress, progress_all, sec_per_item, sa_t))

def total_system_costs_record(txtResultPath, name, hypoZ, hypoZWeighted,
                              hypoTotalGhg, hypoPumpGhg, hypoWWTPGhg, hypoPipeGhg,
                              hypoTotalCosts, hypoPumpCosts, hypoWWTPcosts, hypoPipeCosts,
                              sumFlow, WWTP_number,
                              first_ite=False):
    if first_ite:
        write_mode = "w"
        ...
        write_mode = "a"
    with open(txtResultPath + name + ".txt", write_mode) as f:
        f.write("{},{},".format(hypoZ, hypoZWeighted,) +
                "{},{},{},{},".format(hypoTotalGhg, hypoPumpGhg, hypoWWTPGhg, hypoPipeGhg) +
                "{},{},{},{},".format(hypoTotalCosts, hypoPumpCosts, hypoWWTPcosts, hypoPipeCosts) +
                "{},{}\n".format(sumFlow, WWTP_number)
        )